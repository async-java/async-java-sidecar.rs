...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> SidecarEnvValues {
    SidecarEnvValues {
        bind: lookup("ASYNC_JAVA_SIDECAR_BIND").filter(|value| !value.is_empty()).unwrap_or_else(|| "127.0.0.1:9090".to_string()),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let bind = pick(&["ASYNC_JAVA_SIDECAR_BIND"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("127.0.0.1:9090"));
    if let Some(value) = bind {
        out.insert("ASYNC_JAVA_SIDECAR_BIND".to_string(), value);
    }
    Ok(out)
}
