...
export function loadFrom(lookup: (key: string) => string | undefined): SidecarEnvValues {
  return {
    bind: nonEmpty(lookup("ASYNC_JAVA_SIDECAR_BIND")) ?? "127.0.0.1:9090",
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const bind = pick(["ASYNC_JAVA_SIDECAR_BIND"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "127.0.0.1:9090");
  if (bind !== undefined) out["ASYNC_JAVA_SIDECAR_BIND"] = bind;
  return out;
}
